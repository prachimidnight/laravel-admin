...
class guest extends Model
{
    ...
    public function getallguest($data)
    {
        $query = DB::table('tbl_guest as tg')
            ...
            ->leftJoin('tbl_functioncategories as tfc', 'tg.categories_id', '=', 'tfc.categories_id') // Added category join
            ...
            ->select(
                ...
                'tr.role_id',
                'tr.role_name',
                'tfc.categories_id', // Added categories_id
                'tfc.categories_name', // Added categories_name
                'tfc.categories_date', // Added categories_date
                'tfc.function_tithi', // Added function_tithi
                'co.country_id',
                'co.country_name',
                'ts.state_id',
                'ts.state_name',
        ...
        if (!empty($data['sortby']) && !empty($data['sorttype'])) {
            ...
            $query->orderBy('tg.guest_id', 'DESC'); // Changed to DESC to show newest first
        }
        ...
        // Added categories_id filter
        if (!empty($data['categories_id'])) {
            $query->where('tfc.categories_id', $data['categories_id']);
        }
        ...
        if (!empty($data['search'])) {
            ...
            $query->where(function ($q) use ($searchTerm) {
                $q->where('tg.first_name', 'like', '%' . $searchTerm . '%')
                  ...
                  ->orWhere('tfc.categories_name', 'like', '%' . $searchTerm . '%'); // Added search in category name
            });
        }
